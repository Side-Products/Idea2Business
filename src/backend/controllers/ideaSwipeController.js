...
const generateIdea = catchAsyncErrors(async (req, res, next) => {
	...
	// a max of 5 ideas can be generated by a user in a day so that we do not overload openai's api
	let newIdeasGeneratedByUser = parseInt(req?.cookies?.newIdeasGenerated) || 0;

	if ((req && req.user && req.user.role == "admin") || (newIdeasGeneratedByUser < 5 && randomNumber > 0)) {
		const prompt =
		...
		try {
			...
			try {
				...
				newIdeasGeneratedByUser += 1;
				res.setHeader(
					"Set-Cookie",
					cookie.serialize("newIdeasGenerated", newIdeasGeneratedByUser, {
						httpOnly: true,
